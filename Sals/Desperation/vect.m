function [objective] = vect(x,sourceCoordinates,targetCoordinates,adjMatrix,impossible,xind,yind)

%x is list of all coordinates in the form [x's,y's]
%chargeCoordinates is list of all charging point coordintes, just a
%transformation of x

objective = zeros(size(x,1),1);

for sol=1:size(x,1)
    xind = x(sol,:);
    %Fold x array in half to get coordinates of charging pts.
    chargeCoordinates = [xind(1:length(xind)/2)', xind(length(xind)/2 + 1:length(xind))'];
    ...
    objective(sol) = 10^8*(numInfeasible) + 10^12*(numImpossible-impossible);
end
