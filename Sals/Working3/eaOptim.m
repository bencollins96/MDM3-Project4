function [objective] = eaOptim(x,speedKmh,rangeKm,maxTime,chargeTime,maxJourneyTime,sourceCoordinates,targetCoordinates,adjMatrix,impossible,xind,yind)
...
for target=1:size(targetCoordinates,1)
    ...
    [costs,paths] = dijkstra(adjMatrix,costMatrix,setdiff(1:size(sourceCoordinates),yind(xind==target)), size(sourceCoordinates,1)+target,speedKmh,rangeKm,chargeTime);    
    costs = costs-chargeTime; %minus one charge time because doesn't need to charge when it arrives at target
    for k=1:length(paths)
        if length(paths{k}) == 1
            numInfeasible = numInfeasible + 1;
        end
        if costs(k)>maxJourneyTime
...
objective = 10^6*(numInfeasible) + 10^12*(numImpossible-impossible);
