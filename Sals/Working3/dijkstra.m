...
function [costs,paths] = dijkstra(AorV,xyCorE,SID,FID,rangeKm,speedKmh,chargeTime)
    

    interNodeRange = (rangeKm/speedKmh)*60*60;
        ...
        while any(~isSettled(FID))
            ...
            for kk = 1:length(nodeIndex)
                ...
                if ~isSettled(J)
                    ...
                    if (empty || (jTable(J) > (jTable(I) + c)) ) && c < interNodeRange
                        iTable(J) = jTable(I) + c + chargeTime;
                        if isReverse
                            path{J} = [J path{I}];
                        else
                            path{J} = [path{I} J];
